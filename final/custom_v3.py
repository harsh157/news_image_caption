...
def run_epoch(loader, encoder, model, loss_compute, epoch, scheduler, optimizer, dataset, mode='train'):
    ...
    print(len(loader))
    n_samples = 0
    eval_out = {'bleu-1':0.0,'bleu-2':0.0,'bleu-3':0.0,'bleu-4':0.0,'bleu-1r':0.0}
...
def train(opt):
    ...
    pad_idx = 0
    d_model = 512
    model = transformer_v2.make_model_news(vocab_size+1, num_enc_dec=6, dim_model = d_model)
