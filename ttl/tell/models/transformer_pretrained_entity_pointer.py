...
@Model.register("transformer_pretrained_entity_pointer")
class TransformerPretrainedEntityPointerModel(LoadStateDictWithPrefix, Model):
    def __init__(self,
                 vocab: Vocabulary,
                 decoder: Decoder,
                 criterion: Criterion,
    ...
    def forward(self,  # type: ignore
        ...
        X = self.addtional_attention(decoder_out, contexts)
        #X = self.output_layer(decoder_out, article_ids)
        X = self.output_layer(X, entity_tokens)
        ...
        if not self.training and self.evaluate_mode:
            log_probs, gen_ids = self._generate(
                caption_ids, contexts, article_ids, entity_tokens)
    ...
    def addtional_attention(self, X, contexts):

        #predictors = X[1]['predictors']
        inp_embed = X[1]['inp_embed']
        X = X[0].transpose(0, 1)
        residual = X
        X_entity = self.maybe_layer_norm(
            self.context_attn_lns, X, before=True)
        X_entity, _attn = self.context_attns(
            query=X_entity,
            key=contexts['entity'],
        ...
        X_entity = self.maybe_layer_norm(
            self.context_attn_lns, X_entity, after=True)
        X = X_entity
        ...
        return X, {'attn':_attn, 'predictors':predictors}
    ...
    def _generate(self, caption_ids, contexts, article_ids, entity_tokens):
        ...
        for i in range(gen_len):
            ...
            X = self.addtional_attention(decoder_out, contexts_i)
            #X = self.output_layer(decoder_out, article_ids)
            X = self.output_layer(X, curr_entity_tokens)
