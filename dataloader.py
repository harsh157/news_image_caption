...
def get_sen_embed(args):
    ...
    sen_embed = np.stack(temp_h5['USE'][sen_ix, :]).transpose()
    temp_h5.close()
    return sen_embed
...
class DataLoader:
    ...
    def __init__(self, opt):
        ...
        keys_int = map(lambda x: int(x), self.ix_to_word.keys())
        max_idx = max(keys_int)
        self.ix_to_word[str(max_idx+1)] = '<start>'
        self.ix_to_word[str(max_idx+2)] = '<end>'
        #self.ix_to_word[str(0)] = '<pad>'

        self.vocab_size = len(self.ix_to_word)
        print('vocab size is ', self.vocab_size)
        self.word2idx = {wrd:val for val,wrd in self.ix_to_word.items()}
        ...
        print(self.h5_image_file.root.images)
        for ix in range(len(self.info['images'])):
            img = self.info['images'][ix]
            if np.sum(np.array(self.h5_image_file.root.images[ix, :, :, :])) == 0:
                continue
    ...
    def get_batch_one(self, split):
        ...
        label_batch[i * self.seq_per_img: (i + 1) * self.seq_per_img, self.seq_length + 1] = self.word2idx['<end>']
        label_batch[i * self.seq_per_img: (i + 1) * self.seq_per_img, 0] = self.word2idx['<start>']
        print(label_batch)
        ...
        if self.opt.sentence_embed:
            ...
            sen_embed = np.stack(self.h5_sen_embed_file['USE'][sen_ix, :]).transpose()
