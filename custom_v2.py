...
def run_epoch(loader, encoder, model, loss_compute, epoch, scheduler, optimizer, mode='train'):
    ...
    while True:
        ...
        memory = {'image':images_feats, 'article':sen_embed.float()}
        out = model.forward(memory, batch.trg, 
                            None, batch.trg_mask)
        ...
        iteration += 1
        if mode == 'val':
          outputs = translate_sentence(model, memory, loader, loader.seq_length)#model, image_feats, word_map, max_len
...
import final.transformer_new
reload(final.transformer_new)
...
def train(loader):
    pad_idx = 0
    d_model = 1024
    model = final.transformer_new.make_model_news(loader.vocab_size+1, num_enc_dec=6, dim_model = d_model)
    ...
    encoder.cuda()
    criterion = LabelSmoothing(loader.vocab_size+1, padding_idx=pad_idx, smoothing=0.0)
    criterion.cuda()
...
def translate_sentence(model, image_feats, word_map, max_len):         
    ...
    for i in range(max_len - 1):
        out = model.decode(
            image_feats, None, ys, final.transformer_new.subsequent_mask(ys.size(1)).to(device)
        )
        prob = model.generator(out[:, -1])
        _, next_word = torch.max(prob, dim=1)
        #next_word = next_word.data[0]
