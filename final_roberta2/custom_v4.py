...
def run_epoch(opt, loader, encoder, model, loss_compute, epoch, scheduler, optimizer, dataset, mode='train', size_per_epoch=5000):
    ...
    for target_batch, target_mask, label_batch, image_batch, context_batch, ntokens, metadata in loader:
        ...
        try:
            ...
            iteration += 1
            if mode == 'val':
              outputs = translate_sentence(model, memory, dataset.tokenizer, 100)#model, image_feats, word_map, max_len
...
def train(opt):
    model_path = 'v5/latest.pt'
    sch_path =  'v5/scheduler.pth.tar'
    perf_pkl = 'v5/best_model.pkl'
    info_pkl = 'v5/model_info.pkl'
    best_model_path = 'v5/best.pt'
    ...
    model = transformer_v2.make_model_news(vocab_size+1, num_enc_dec=6, dim_model = d_model, dim_feedfwd=4096, img_dim=1024, sent_dim=1024)
