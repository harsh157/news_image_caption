...
def load_roberta(i,context, metadata, roberta, max_len=512):
    c = context[i]
    article_id = metadata[i]['article_id']
    embed_path = f'/mnt/disks/embeds/GoodNews/roberta_embeddings/{article_id}.npy'
    embed_pkl = f'/mnt/disks/embeds/GoodNews/roberta_embeddings/{article_id}.pkl'
    #print(embed_path)
    tokens = roberta.encode(c).tolist()
    ...
    if os.path.exists(embed_path):
        try:
            ...
            try:
                with open(embed_pkl, 'rb') as pfile:
   ...
   # print(features.shape)
    ...
    with open(embed_path, 'wb') as pfile:
        nfeat = features.cpu().detach().numpy()
        np.save(pfile, nfeat)
    return features, pad_x
